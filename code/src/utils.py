...
def models_load(args, data):
    ...
    if args.model=='FM':
        ...
        raise ValueError('MODEL is not exist : select model in [FM,FFM,NCF,WDN,DCN,CNN_FM,DeepCoNN,DEEP_FM]')
    return model
...
class Setting:
    ...
    def get_log_path(self, args, i=0):
        '''
        [description]
        ...
        '''
        if args.kfold:
            path = self.make_dir(f'./log/{self.save_time}_{args.model}/')
        else:    
            str_ = f'Fold {i}'
